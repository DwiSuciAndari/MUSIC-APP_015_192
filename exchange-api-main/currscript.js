...
let currLink, currLink2, currLink3, currLink3Key
if (process.env.CI) {
  ...
  [currLink, currLink2, currLink3, currLink3Key] = fs.readFileSync(path.join(__dirname, 'links.ini')).toString().split(/\r?\n/).map(e => e.trim())
}
// curr means currency
...
currDataObj3 = 'eur' in currDataObj3 ? toLowerCaseKeysBaseCurr(currDataObj3, 1/currDataObj3['eur']) : {}
  ...
  let CurrJSON = { ...currDataObj, ...currDataObj3, eur: 1 }
  
  // Filter khusus untuk IDR, USD, Ringgit (MYR), Euro (EUR), dan Yen (JPY)
  let allowedCurrencies = ['idr', 'usd', 'myr', 'eur', 'jpy']
  let filteredJSON = {}
  for (let key of allowedCurrencies) {
    if (CurrJSON[key]) {
      filteredJSON[key] = CurrJSON[key]
    }
  }

  // return sorted object
  return sortObjByKeys(filteredJSON)
}
